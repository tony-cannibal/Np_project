...
def checklist_info(df):
    ...
    desmalle = [df[(df['NP'] == i )&(df['Nº de circuito'].str.endswith('#'))]['Nº de circuito'].count() * 2
            for i in part_num]
    twist_sld = desmalle
    df = pd.DataFrame(list(zip(part_num, qty, corte, riv, sl, joint_sld, desmalle, twist_sld)),
                      columns=['PN', 'QTY', 'Corte', 'RIVIAN', 'SLD', 'JOINT SLD', 'DESMALLE', 'TWIST MALLA'])
    return df
...
def postp_info(df):
    ...
    termo_sld =  [df[(df['NP'] == i ) & (df['Nº de circuito'].str.endswith('#')) & (df['Maquina'].str.startswith('H'))]['Nº de circuito'].count()
             for i in part_num]

    termo = [df[(df['NP'] == i ) & (df['Maquina'].str.startswith('H'))]['Nº de circuito'].count()
             for i in part_num]
    df = pd.DataFrame(list(zip(part_num, twist, sello, desforre_medio, desforre_punta, encinte_auto, inser_tubo, termo, termo_sld)),
                  columns = ['PN', 'TWIST', 'SELLO', 'DESFORRE MEDIO', 'DESFORRE DE PUNTA',
                             'ENCINTE AUTOMATICO', 'INSERCION DE TUBO', 'TERMO', 'TERMO SLD'])
    return df
